...
function! s:start(args, ctx, x) abort
  ...
  let l:state = {
        ...
        \ 'finished': 0,
        \ 'has_error': 0,
        \ 'on_error': wilder#pipeline#register_func({_, x -> s:on_error(l:state, x)}),
  ...
  while l:i < l:len
    ...
    let l:on_finish = {i -> wilder#pipeline#register_func({_, x -> s:on_finish(l:state, i, x)})}(l:i)
...
function! s:on_error(state, x) abort
  ...
  if a:state.finished >= len(a:state.results)
    ...
    for l:f in a:state.on_finish
      call wilder#pipeline#unregister_func(l:f)
    endfor
  endif
...
function! s:on_finish(state, index, x) abort
  ...
  if a:state.finished >= len(a:state.results)
    ...
    call wilder#pipeline#do(a:state.original_ctx, a:state.results)
  endif
endfunction
