...
function s:concat_start(args, ctx, x)
  ...
  let l:args.on_error = wildsearch#pipeline#register_func({ctx, x -> s:concat_error(l:args, ctx, x)})
  let l:args.on_finish = wildsearch#pipeline#register_func({ctx, x -> s:concat_finish(l:args, ctx, x)})
