...
function! wild#render#set_option(key, value) abort
  let s:opts[a:key] = a:value
endfunction

function! wild#render#set_options(opts) abort
  let s:opts = extend(s:opts, a:opts)
endfunction

function! wild#render#get_option(key) abort
  return s:opts[a:key]
endfunction

function! wild#render#components_len(components, ctx, xs) abort
  let l:len = 0
...
function! wild#render#init() abort
  ...
  call wild#render#components_pre_hook(wild#render#get_components(), {})
...
function! wild#render#finish() abort
  call wild#render#components_post_hook(wild#render#get_components(), {})
endfunction

function! wild#render#components_pre_hook(components, ctx) abort
  for l:Component in a:components
...
function! wild#render#components_post_hook(components, ctx) abort
  for l:Component in a:components
...
function! wild#render#make_page(ctx, xs, page, direction, has_resized) abort
  if empty(a:xs)
    return [-1, -1]
  endif
...
function! wild#render#components_need_redraw(components, ctx, x) abort
  for l:Component in a:components
...
function! wild#render#draw(left, right, ctx, xs) abort
  let l:res = ''

  let l:res .= wild#render#components_draw(a:left, a:ctx, a:xs)
  let l:res .= s:draw_xs(a:ctx, a:xs)
  let l:res .= wild#render#components_draw(a:right, a:ctx, a:xs)

  return l:res
endfunction

function! wild#render#draw_error(left, right, ctx, error) abort
  let l:res = ''

  let l:res .= wild#render#components_draw(a:left, a:ctx, [])
  let l:res .= s:draw_error(a:ctx, a:error)
  let l:res .= wild#render#components_draw(a:right, a:ctx, [])
...
function! wild#render#components_draw(components, ctx, xs) abort
  let l:res = ''
...
function! wild#render#set_components(args) abort
  let s:left = get(a:args, 'left', [])
  let s:right = get(a:args, 'right', [])
endfunction

function! wild#render#get_components(...) abort
  if !exists('s:left') && !exists('s:right')
    call wild#render#set_components(wild#render#default())
  endif
...
function! wild#render#make_hl(name, args) abort
...
function! wild#render#get_colors(group) abort
  if has('nvim')
    return wild#render#get_colors_nvim(a:group)
  else
    return wild#render#get_colors_vim(a:group)
  endif
endfunction

function! wild#render#get_colors_nvim(group) abort
  try
...
function! wild#render#get_colors_vim(group) abort abort
  try
    ...
    if len(l:link_matches) > 0 " follow the link
      return wild#render#get_colors_vim(l:link_matches[1])
    endif
...
function! wild#render#default() abort
  return {
        \ 'left': [wild#previous_arrow()],
        \ 'right': [wild#next_arrow()],
        \ }
endfunction
