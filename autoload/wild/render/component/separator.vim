...
function! wild#render#component#separator#make(str, fg, bg, ...) abort
  if a:0 == 0
    let l:key = s:index
    let s:index += 1
  else
...
function! s:hl(name, from, to) abort
  let l:from_hl = wild#render#get_colors(a:from)
  let l:to_hl = wild#render#get_colors(a:to)
  ...
  call wild#render#make_hl(a:name, l:colors)
endfunction
