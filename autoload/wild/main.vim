...
let s:opts = {
      ...
      \ 'hooks': {
      \    'pre': 'wild#main#save_statusline',
      \    'post': 'wild#main#restore_statusline',
      \  },
      \ 'num_workers': 2,
      \ }

function! wild#main#set_option(key, value) abort
  let s:opts[a:key] = a:value
endfunction

function! wild#main#set_options(opts) abort
  let s:opts = extend(s:opts, a:opts)
endfunction

function! wild#main#get_option(key) abort
  return s:opts[a:key]
endfunction

function! wild#main#in_mode() abort
  return index(s:opts.modes, getcmdtype()) >= 0
endfunction

function! wild#main#in_context() abort
  return wild#main#in_mode() && !s:hidden && s:enabled
endfunction

function! wild#main#enable_cmdline_enter() abort
  if !exists('#WildsearchCmdlineEnter')
    augroup WildsearchCmdlineEnter
      autocmd!
      autocmd CmdlineEnter * call wild#main#start_auto()
    augroup END
  endif
endfunction

function! wild#main#disable_cmdline_enter() abort
  if exists('#WildsearchCmdlineEnter')
    augroup WildsearchCmdlineEnter
      autocmd!
    augroup END
...
function! s:start(check) abort
  ...
  if has('nvim') && !s:init
    let s:init = 1
    call _wild_init({'num_workers': s:opts.num_workers})
  endif
  ...
  if s:auto && !exists('#WildsearchCmdlineLeave')
    augroup WildsearchCmdlineLeave
      autocmd!
      autocmd CmdlineLeave * call wild#main#stop()
    augroup END
  endif
...
function! s:pre_hook() abort
  ...
  call wild#render#init()
endfunction

function! s:post_hook() abort
  call wild#render#finish()
...
function! s:do(check) abort
  ...
  if s:force || !l:has_completion && (l:input_changed || l:is_new_input)
    ...
    let l:ctx = {
        \ 'input': l:input,
        \ 'on_finish': 'wild#main#on_finish',
        \ 'on_error': 'wild#main#on_error',
        \ 'run_id': s:run_id,
        \ }

    let s:run_id += 1

    call wild#pipeline#start(l:ctx, l:input)
  endif
  ...
  if !s:draw_done && (l:is_new_input ||
        \ wild#render#components_need_redraw(wild#render#get_components(), l:ctx, s:candidates))
    call s:draw()
  endif
...
function! wild#main#on_finish(ctx, x) abort
  if !s:active || !s:enabled
    return
  endif
...
function! wild#main#on_error(ctx, x) abort
  if !s:active || !s:enabled
    return
  endif
...
function! s:draw(...) abort
  ...
  let l:space_used = wild#render#components_len(
        \ l:left_components + l:right_components,
        \ l:ctx, l:xs)
  let l:ctx.space = winwidth(0) - l:space_used

  let s:page = wild#render#make_page(l:ctx, l:xs, s:page, l:direction, l:has_resized)
  let l:ctx.page = s:page

  if l:has_error
    let l:statusline = wild#render#draw_error(
          \ l:left_components, l:right_components,
          \ l:ctx, s:error)
  else
    let l:statusline = wild#render#draw(
          \ l:left_components, l:right_components,
          \ l:ctx, l:xs)
  endif
...
function! wild#main#next() abort
  return wild#main#step(1)
endfunction

function! wild#main#previous() abort
  return wild#main#step(-1)
endfunction

function! wild#main#step(num_steps) abort
  if !s:enabled
...
function! wild#main#accept_completion() abort
...
function! wild#main#reject_completion() abort
  if exists('s:menus')
    if !empty(s:menus)
      let s:force = 1
...
function! wild#main#save_statusline() abort
...
function! wild#main#restore_statusline() abort
