function! wild#pipeline#component#branch#make(args) abort
  if len(a:args) == 0
    return {_, x -> v:false}
  endif
...
function! s:branch_start(args, ctx, x) abort
  ...
  let l:state.on_error = wild#pipeline#register_func({ctx, x -> s:branch_error(l:state, ctx, x)})
  let l:state.on_finish = wild#pipeline#register_func({ctx, x -> s:branch_finish(l:state, ctx, x)})
...
function! s:branch_error(state, ctx, x) abort
  ...
  call wild#pipeline#unregister_func(a:state.on_finish)
...
function! s:branch_finish(state, ctx, x) abort
  if a:x isnot v:false
    ...
    call wild#pipeline#do(a:state.original_ctx, a:x)
    return
  endif
  ...
  if a:state.index >= len(a:state.fs_list)
    ...
    call wild#pipeline#do(a:state.original_ctx, v:false)
    return
  endif
  ...
  call wild#pipeline#do(l:ctx, a:state.original_x)
endfunction
